...
def raycast_cone(dg, ro, head_quat, gaze_quat):
    ...
    return closest_isect, ray_isect, np.array(all_norms)
...
def register():
    bpy.types.Scene.rec_dir = bpy.props.StringProperty(
        name="rec_dir",
        description="Directory with Neon recording to animate",
        default="",
    )
    bpy.types.Scene.gaze_cone_intersect = bpy.props.BoolProperty(
        name="gaze_cone_intersect",
    ...
    if kc:
        ...
        kmi = km.keymap_items.new(
            OperatorAnimateNeon.bl_idname, "T", "PRESS", ctrl=True, shift=True
        )
...
class OperatorImportMeshes(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.import_scene.gltf(
            filepath=str(
                addon_file_path / "tag_aligner_assets" / "tag_aligner_pl_objs.gltf"
            )
        )
...
class OperatorAnimateNeon(bpy.types.Operator):
    ...
    def __init__(self):
        ...
        self.gaze_df["timestamp [s]"] = (
            self.gaze_df["timestamp [ns]"] - self.start_ts
        ) * 1e-9
        ...
        self.data_start_s = (
            self.head_start_s
            if self.head_start_s < self.gaze_start_s
            else self.gaze_start_s
        )
        ...
        self.report(
            {"INFO"},
            "Tag Aligner: Temporal context calculated. Preparing keyframe loop...",
        )
    ...
    def apply_curve_material(self):
        ...
        if bpy.data.materials.get("curve_material") is None:
            ...
            bpy.data.materials["curve_material"].node_tree.nodes[
                "Principled BSDF"
            ].inputs[0].default_value = (1, 0.236721, 0.0176297, 1)
            bpy.data.materials["curve_material"].node_tree.nodes[
                "Principled BSDF"
            ].inputs[26].default_value = (1, 0.111108, 0.0123341, 1)
            # bpy.data.materials["curve_material"].node_tree.nodes["Principled BSDF"].inputs[27].default_value = 0.8
            bpy.data.materials["curve_material"].node_tree.nodes[
                "Principled BSDF"
            ].inputs[2].default_value = 1
        else:
    ...
    def init_fcurves(self, context):
        ...
        for obj in self.pl_objs:
            if obj is context.scene.objects["head_trajectory"]:
                ...
                obj.data.animation_data.action = bpy.data.actions.new(
                    name=obj.name + "Action"
                )

                bevel_end_fcurve = obj.data.animation_data.action.fcurves.new(
                ...
                loc_fcurves = [
                    obj.animation_data.action.fcurves.new("location", index=index)
                    for index in range(3)
                ]
                rot_fcurves = [
                    obj.animation_data.action.fcurves.new(
                        "rotation_quaternion", index=index
                    )
                    for index in range(4)
                ]
                scale_fcurves = [
                    obj.animation_data.action.fcurves.new("scale", index=index)
                    for index in range(3)
                ]
    ...
    def insert_fcurve(self, obj, frame_num, location, rotation, scale):
        ...
        for lc in range(3):
            obj_fcurves["location"][lc].keyframe_points[frame_num].co_ui = [
                frame_num,
                location[lc],
            ]

        for rc in range(4):
            obj_fcurves["rotation"][rc].keyframe_points[frame_num].co_ui = [
                frame_num,
                rotation[rc],
            ]

        for sc in range(3):
            obj_fcurves["scale"][sc].keyframe_points[frame_num].co_ui = [
                frame_num,
                scale[sc],
            ]
    ...
    def execute(self, context):
        ...
        self.report(
            {"INFO"},
            "Tag Aligner: Curve parameters and depsgraph prepared. Entering keyframe loop...",
        )
        ...
        for frame_num in range(context.scene.frame_end):
            if frame_num < self.n_buffer_frames:
                ...
                self.insert_fcurve(
                    self.JAN, frame_num, coordinates, rot, [0.001, 0.001, 0.001]
                )
                ...
                self.insert_fcurve(
                    self.gzray, frame_num, coordinates, gzray_rot, [0.8, 0.8, 0.0]
                )
                ...
                gaze_idxs = (
                    self.gaze_df["timestamp [s]"]
                    > self.poses_df["start_timestamp"][pose_idx]
                ) & (
                    self.gaze_df["timestamp [s]"]
                    < self.poses_df["end_timestamp"][pose_idx]
                )
                ...
                if bpy.context.scene.gaze_cone_intersect:
                    closest_isect, ray_isect, all_norms = raycast_cone(
                        dg, ro, head_quat, gaze_quat
                    )
                else:
                    rd = head_quat @ gaze_quat @ Vector((0.0, 1.0, 0.0))
                    rd = rd / np.linalg.norm(rd)
                    closest_isect = bpy.context.scene.ray_cast(dg, ro, rd)
                ...
                if closest_isect is not None:
                    ...
                    #                    n = isect[2]
                    n = Vector(np.mean(all_norms, axis=0))
                ...
                self.head_fcurve.keyframe_points[frame_num].co_ui = [
                    frame_num,
                    pose_idx / len(self.poses_df),
                ]
                ...
                if closest_isect is not None:
                    ...
                    if ray_isect is None:
                        gzcirc_rot = head_quat @ gaze_quat @ self.init_quat_gzcirc
                    else:
                        gzcirc_rot = norm_quat @ self.init_quat_gzcirc
        ...
        self.report(
            {"INFO"},
            "Tag Aligner: Prepared {} keyframes.".format(context.scene.frame_end),
        )
